...
// it is used for mapping skills with percentages
const dict = {
  photography: '100%',
  photoshop: '90%',
  illustrator: '80%',
...
// initializing counters for each skill
let counter0 = 0;
let counter1 = 0;
let counter2 = 0;
let counter3 = 0;
...
// Adding event listener for scrolling effect
window.addEventListener('scroll', scroller);

// Functions
// Increases all percentages in 6 progress bars in every 5 millisecons
function animate() {
...
// Depending on screen resolution and mode (portrait/landscape)
// this func fires off animation of progress bars
function scroller() {
  const {scrollTop, scrollHeight, clientHeight} = document.documentElement;

  if (width > height && height < 376) {
    //    800          375            2100
    if (scrollTop + clientHeight >= scrollHeight - 925) {
      animate();
